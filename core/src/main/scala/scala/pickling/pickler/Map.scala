...
trait MapPicklers extends GeneratorRegistry {
  ...
  locally {
    ...
    val cbf = implicitly[CanBuildFrom[Map[Any, Any], (Any, Any), Map[Any, Any]]]
...
trait ImmutableSortedMapPicklers {
  ...
  implicit def immutableSortedMapPickler[K: FastTypeTag, V: FastTypeTag]
    (implicit elemPickler: Pickler[(K, V)], elemUnpickler: Unpickler[(K, V)],
     pairTag: FastTypeTag[(K, V)], collTag: FastTypeTag[immutable.SortedMap[K, V]],
...
trait MutableMapPicklers extends GeneratorRegistry {
  ...
  implicit def mutableMapPickler[K: FastTypeTag, V: FastTypeTag]
    (implicit elemPickler: Pickler[(K, V)], elemUnpickler: Unpickler[(K, V)],
     pairTag: FastTypeTag[(K, V)], collTag: FastTypeTag[Mmap[K, V]],
     cbf: CanBuildFrom[Mmap[K, V], (K, V), Mmap[K, V]]
     ): Pickler[Mmap[K, V]] with Unpickler[Mmap[K, V]] =
    MapPickler[K, V, Mmap]

  locally {

    val cbf = implicitly[CanBuildFrom[Mmap[Any, Any], (Any, Any), Mmap[Any,Any]]]
